...
この力を身につけるためには、実際の過去問を繰り返し解き、記述形式に慣れることが欠かせません。  
しかし、独学では、自分が作成した答案について客観的なフィードバックを得る機会が少なく、答案のどこが不足しているのか、どのように改善すればよいのかを判断しにくいという課題があります。
...
### フロントエンドとバックエンドを分離したSPA構成

Laravelによるサーバーサイド実装にとどまらず、フロントエンドにReact・TypeScriptを採用し、LaravelをAPIサーバーとするSPAとして開発しました。  
フロントエンドでは画面表示や状態管理、バックエンドでは認証・認可、データの永続化、AI処理などを担当し、APIを境界として責務を分離しています。  
また、過去問PDFの閲覧、答案入力、AI添削を一つの画面上で行えるようにし、AI処理中もPDFや入力内容を保持することで、学習中の文脈を維持したまま結果を確認できるようにしています。
...
### Redisを利用したAI処理の多重実行防止

AI添削や質問への回答は数十秒かかる場合があり、二重送信されると同じ処理が並列実行され、API利用料の増加やデータ不整合につながります。フロントエンドで送信ボタンを無効化するだけでは、別タブや直接APIを呼び出した場合には防げないため、Redis Lockを利用してユーザー単位でバックエンド側でも排他制御しています。

### DockerとGitHub Actionsによる開発・デプロイ環境

Laravel、Caddy、MySQL、RedisをDocker Composeで構成し、開発環境と本番環境の差を小さくしています。  
mainブランチへのpushを契機に、依存関係の脆弱性監査、静的解析、テスト、フロントエンドのビルドを実行し、すべて成功した場合のみDockerイメージを作成しています。テストに失敗したコードがそのまま本番用イメージとして配布されない構成にしました。
